...
const numberNames = new Map([[5, 'five'], [6, 'six'], [8, 'eight'], [10, 'ten']]);
const numberName = value => numberNames.get(value) || String(value);

function slideSections(source) {
  const chunks = source.replace(/^---\n[\s\S]*?\n---\n/, '').split(/\n---\n/);
  const sections = [];
  for (let index = 0; index < chunks.length; index++) {
    const possibleFrontmatter = chunks[index].trim();
    if (index > 0 && !possibleFrontmatter.includes('<!--') && /^[a-z][\w-]*:/im.test(possibleFrontmatter)) {
      index++;
      if (index >= chunks.length) throw new Error('slide frontmatter has no slide body');
    }
    sections.push(chunks[index]);
  }
  return sections;
}
...
export function writeNotes(output) {
  ...
  for (const deck of decks) {
    ...
    if (sections.length !== deck.slides) throw new Error(`${deck.slug}: expected ${deck.slides} slides`);
    ...
    const slideCount = numberName(deck.slides);
    const duration = numberName(deck.minutes);
    const markdown = `# ${deck.title} — speaker notes\n\n${slideCount[0].toUpperCase()}${slideCount.slice(1)} slides · approximately ${duration} minutes · ${words} spoken words.\n\nEdit the closing comment in each slide in ${deck.slug}/slides.md; these notes are generated from that source.\n\n` + notes.map(n => `## ${n.number}. ${n.title}\n\n${n.note}\n`).join('\n');
    ...
    all.push(markdown);
    console.log(`${deck.slug}: ${deck.slides} slides, ${words} spoken words`);
  }
