...
The no-model boundary doctor currently selects `google/gemma-4-31b-it` with `minimal` thinking because that model was used for the earlier low-reasoning calibration. This verifies the provider-child configuration; it does not freeze the live comparison model. Choosing a Gemini, Claude, or Codex model instead changes the target of the experiment and requires a new committed live configuration and clean doctor check.
...
3. Dave is confirmed as the disclosed-author rater; confirm the independent rater (Dan is the current candidate), agree the deadline and model-specific spend ceiling, and freeze exact arm prompts and allocation schedule.
